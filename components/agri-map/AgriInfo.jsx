...
const AgriInfo = () => {
  ...
  useEffect(() => {
    if (curCategory && !curProductList) {
      ...
      fetch(`https://bobusang.herokuapp.com/${curCategory}s/localEngName/${curLocation}`)
      ...
      if (curProduct && !curCitiesOnProduct) {
        fetch(`https://bobusang.herokuapp.com/${curCategory}s/itemNameAndLocalEngName/${curProduct}/${curLocation}`)
        .then(response => response.json())
        .then(data => {
            const curList = [];
